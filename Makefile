...
.PHONY: mydata
mydata: install ## Run experiments with real data only
	# Execute only the scaling_small experiment (uses real data)
	@uv run python -c "import sys; from pathlib import Path; sys.path.append((Path('experiments')).as_posix()); from experiments.scaling_small import main; main()"

.PHONY: results
results: install ## Show results from existing backtest
	# Display comprehensive results from the latest backtest
	@uv run python show_results.py

.PHONY: mydata-results
mydata-results: mydata results ## Run backtest with real data and show results
