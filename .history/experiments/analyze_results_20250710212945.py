...
def generate_comprehensive_dashboard(result: BacktestResult, result_name: str, 
    ...
    plt.yticks(range(len(top_changes)), list(top_changes.index))
...
if __name__ == "__main__":
    ...
    try:
        from extract_weights import load_extracted_weights
        initial_weights, final_weights, target_weights = load_extracted_weights()
        if initial_weights is not None:
            logger.info("Successfully loaded extracted weights for analysis")
    except ImportError:
        logger.warning("Could not import extract_weights module. Using None for weights.")
