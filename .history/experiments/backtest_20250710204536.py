...
class BacktestResult:
    ...
    def periods_per_year(self) -> float:
        days_diff = (self.history[-1] - self.history[0]).days
        if days_diff == 0:
            # If we have only one day or same start/end date, assume 252 trading days per year
            return 252.0
        return len(self.history) / (days_diff / 365.25)
