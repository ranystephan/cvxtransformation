...
def run_backtest(
    ...
    result_index = pd.Index(result_timestamps)

    # We need to adjust the daily_target_weights_history to match the main results' length
    # It won't have a "Day 0" value, so we pad it.
    if initial_weights is not None:
        daily_target_weights_history.insert(0, np.full(n_assets, np.nan))
