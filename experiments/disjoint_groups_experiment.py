...
def create_minimum_variance_portfolio(asset_indices: List[int], n_assets: int, target_risk: float = 0.10) -> np.ndarray:
    ...
    weight_per_asset = target_risk / len(asset_indices)
    
    for i in asset_indices:
        w[i] = weight_per_asset
...
def run_disjoint_groups_experiment():
    ...
    for scenario in scenarios:
        ...
        w_initial = create_minimum_variance_portfolio(initial_assets, n_assets, scenario['initial_risk'])
        w_target = create_minimum_variance_portfolio(target_assets, n_assets, scenario['target_risk'])
